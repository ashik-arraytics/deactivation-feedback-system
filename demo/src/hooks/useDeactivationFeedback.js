...
const useDeactivationModal = ({ 
  ...
  useEffect(() => {
    ...
    const deactivateLink = Array.from(document.querySelectorAll('a[href*="plugins.php?action=deactivate"]'))
      .find(link => link.href.includes('plugin=poptics%2Fpoptics.php') || link.id === `deactivate_${pluginName}`);
    
    console.log("✨ ~ findDeactivationLink ~ deactivateLink:", deactivateLink)
      if (!deactivateLink) {
    ...
    if (window.location.href.includes('/')) { // as we are testing on react app localhost
